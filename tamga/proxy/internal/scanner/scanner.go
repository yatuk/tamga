...
// Refreshable is an optional interface for scanners that can hot-reload
// configuration on policy changes without restarting. Check with type assertion.
type Refreshable interface {
	Refresh()
}

// StatefulScanner is an optional interface for scanners with background
// goroutines that need explicit start/stop lifecycle management.
...
// dependencies (Redis, file watchers, gRPC connections). Used by the
// health endpoint to report per-scanner dependency status.
...
// All returns a snapshot of all registered scanners. Safe for concurrent use.
// Callers can type-assert each Scanner against optional interfaces
// (Refreshable, StatefulScanner, HealthReporter) without holding the lock.
