...
func main() {
	...
	var opStateScanner *operator_state.OperatorStateScanner
	...
	if sw, err := policy.WatchPolicy(cfg.PolicyPath, policyStore, func() {
		...
		if opStateScanner != nil {
			p := policyStore.GetPolicy()
			assertions, err := operator_state.LoadAssertionsFromPolicy(p.OperatorState)
			if err != nil {
				log.Warn().Err(err).Msg("opstate assertion reload failed; keeping previous rules")
			} else {
				opStateScanner.UpdateAssertions(assertions)
				log.Info().Int("assertions", len(assertions)).Msg("opstate assertions reloaded")
			}
		}
		if tierEnforcer != nil {
	...
	// ── Jugeni operator-state scanner (optional) ──────────────────────────
	var opStateWatcher *operator_state.Watcher
	var opStateProjection *operator_state.Projection
	if cfg.OperatorStateDecisionsPath != "" || cfg.OperatorStateNotesPath != "" {
		opStateProjection = operator_state.NewProjection()

		// Attach Redis for distributed state persistence (optional, fail-open).
		if rdx.Enabled() {
			redisStore := operator_state.NewRedisStore(rdx)
			opStateProjection.SetRedisStore(redisStore)
		}

		opStateCfg := operator_state.Config{
			DecisionsPath: cfg.OperatorStateDecisionsPath,
			NotesPath:     cfg.OperatorStateNotesPath,
			PollInterval:  time.Duration(cfg.OperatorStatePollIntervalMs) * time.Millisecond,
			Enabled:       true,
		}

		var watcherErr error
		opStateWatcher, watcherErr = operator_state.NewWatcher(opStateCfg,
			func(ev operator_state.DecisionEvent) { opStateProjection.ApplyDecision(ev) },
			func(ev operator_state.NoteEvent) { opStateProjection.ApplyNote(ev) },
		)
		if watcherErr != nil {
			log.Warn().Err(watcherErr).Msg("opstate watcher failed to start; operator-state scanner disabled")
		} else {
			go func() {
				if err := opStateWatcher.Start(context.Background()); err != nil {
					log.Error().Err(err).Msg("opstate watcher stopped with error")
				}
			}()
			// Wait for initial replay to complete (bounded wait).
			if err := opStateWatcher.WaitInitial(); err != nil {
				log.Warn().Err(err).Msg("opstate initial replay completed with warnings")
			}
			dCount, nCount := opStateProjection.Stats()
			log.Info().
				Int("decisions", dCount).
				Int("notes", nCount).
				Msg("opstate projection initialised")

			// Load assertion rules from policy.
			assertions, err := operator_state.LoadAssertionsFromPolicy(pol.OperatorState)
			if err != nil {
				log.Warn().Err(err).Msg("opstate assertion rules failed to load; scanner runs without assertions")
			}
			opStateScanner = operator_state.NewOperatorStateScanner(opStateProjection, assertions)

			// Attach Redis for fast-path state lookups (<0.8ms target).
			if rdx.Enabled() {
				redisStore := operator_state.NewRedisStore(rdx)
				opStateScanner.SetRedisStore(redisStore)
				redisStore.SeedFromProjection(context.Background(), opStateProjection)
			}

			registry.Register(opStateScanner)
			registry.SetSpeed("operator_state", scanner.SpeedFast)
			log.Info().Int("assertions", len(assertions)).Msg("opstate operator-state scanner registered")
		}
	}
	...
	if opStateWatcher != nil {
		opStateWatcher.Stop()
		log.Info().Msg("opstate watcher stopped")
	}
