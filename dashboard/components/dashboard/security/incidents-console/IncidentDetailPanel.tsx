...
export function IncidentDetailPanel({ event, m, onFpClick }: Props) {
  ...
  if (!event) {
    return (
      <div className="flex h-full items-center justify-center p-6 text-center">
        <div>
          <Shield className="mx-auto h-8 w-8 text-fg-muted" />
          <p className="mt-2 text-[11px] text-fg-muted">
            Select an incident from the queue to inspect
          </p>
          <p className="mt-1 text-[10px] text-fg-muted dark:text-fg-subtle">
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="border-b border-border px-3 py-2">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px]">
          <span className="inline-flex items-center gap-1.5">
            ...
            <span className="font-mono text-fg dark:text-fg">{event.request_id.slice(0, 16)}</span>
          </span>
          <span className="inline-flex items-center gap-1.5">
            <Clock3 className="h-3 w-3 text-fg-subtle" />
            <span className="text-fg-muted">{relativeTime(event.timestamp)}</span>
          </span>
          <span className="inline-flex items-center gap-1.5">
      ...
      {findings.length > 0 && (
        <div className="border-b border-border px-3 py-2">
          <div className="text-[10px] uppercase tracking-[0.18em] text-fg-muted mb-1.5">
          ...
          <div className="space-y-1">
            {findings.map((f, i) => (
              <div
                key={i}
                className="flex items-center justify-between rounded-sm border border-border bg-surface-subtle px-2 py-1.5 text-[11px]"
              >
                <span className="flex items-center gap-2">
                  <span className="font-mono text-fg dark:text-fg">
                    {f.type}:{f.category}
                  </span>
                  {f.match && (
                    <span className="font-mono text-[10px] text-fg-subtle truncate max-w-[200px]">
                      &ldquo;{f.match}&rdquo;
                    </span>
                  )}
                </span>
      ...
      <div className="border-t border-border px-3 py-2">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="ml-auto text-[10px] text-fg-muted flex gap-2">
