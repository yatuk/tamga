...
func TestOperatorStateScanner_UnknownDecision(t *testing.T) {
	...
	// Default-deny: a governed decision id missing from the audit log is
	// denied, not silently allowed.
	if len(findings) != 1 {
		t.Fatalf("expected 1 default-deny finding for unknown decision, got %d", len(findings))
	}
	if findings[0].Category != "unknown_decision_ref" {
		t.Errorf("Category = %q, want unknown_decision_ref", findings[0].Category)
	}
	if findings[0].ConfidenceScore == nil || findings[0].ConfidenceScore.Action != "BLOCK" {
		t.Errorf("expected BLOCK confidence action for unknown decision ref")
	}
}

func TestOperatorStateScanner_UnknownDecision_AllowToggle(t *testing.T) {
	p := NewProjection()
	...
	if len(findings) != 0 {
		t.Errorf("expected 0 findings with on_unknown_ref=allow, got %d", len(findings))
	}
}

func TestOperatorStateScanner_UnknownDecision_Ungoverned(t *testing.T) {
	// With no rules at all, unknown decision ids are out of governance scope
	// and pass silently even under default-deny.
	...
	if len(findings) != 0 {
		t.Errorf("expected 0 findings for ungoverned unknown decision, got %d", len(findings))
	}
}
...
func TestOperatorStateScanner_Name(t *testing.T) {
	s := NewOperatorStateScanner(NewProjection(), nil)
	if s.Name() != "operator_state" {
		t.Errorf("Name = %q, want operator_state", s.Name())
	}
}
