...
type Config struct {
	...
	// RedisEnabled gates Redis write-through for the decision store
	// (TAMGA_OPERATOR_STATE_REDIS, default true). Effective only when the
	// proxy's Redis client is itself configured and reachable.
	RedisEnabled bool
...
func LoadConfig() Config {
	cfg := Config{
		...
		RedisEnabled:  envOrDefault("TAMGA_OPERATOR_STATE_REDIS", "true") != "false",
	}
