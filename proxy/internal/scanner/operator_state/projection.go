...
// Timestamps are parsed before comparison — the schema preserves local UTC
// offsets, so a lexicographic string sort would misorder mixed-offset logs
// (e.g. "+02:00" vs "Z"). Unparseable timestamps fall back to string order.
func SortDecisionEvents(events []DecisionEvent) {
	sort.SliceStable(events, func(i, j int) bool {
		ti, ei := events[i].ParseTimestamp()
		tj, ej := events[j].ParseTimestamp()
		if ei != nil || ej != nil {
...
// See SortDecisionEvents for the mixed-offset rationale.
